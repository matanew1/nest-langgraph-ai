...
const logger = new Logger("Clarification");
...
export async function clarificationNode(
  ...
  let clarificationMessage = "The agent needs more information to proceed. Please rephrase your request or provide additional details.";

  if (lastError && lastError.atPhase === "supervisor" && lastError.details && Array.isArray(lastError.details.missing_capabilities)) {
  ...
  return {
    ...
    // Clear errors after attempting recovery
    errors: [],
  };
}