...
export async function planValidatorNode(
  ...
  if (steps.length === 0) {
    ...
    return {
      ...
      errors: [{ code: 'invariant_violation', message: 'Empty plan', atPhase: 'validate_plan' }],
    };
  }
  ...
  if (steps.length > AGENT_PLAN_LIMITS.maxSteps) {
    ...
    return {
      ...
      errors: [{ code: 'invariant_violation', message: `Plan too long`, atPhase: 'validate_plan' }],
    };
  }
  ...
  for (const step of steps) {
    if (step.tool === 'file_patch') {
      const input = step.input as { path: string; find: string; replace?: string };
      
      if (!input.path || !input.find) {
        logPhaseEnd('PLAN_VALIDATOR', `FAILED: invalid file_patch params (step ${step.step_id})`, elapsed());
        return {
          phase: 'fatal',
          finalAnswer: `Invalid file_patch params in step ${step.step_id}.`,
          errors: [{ code: 'invariant_violation', message: 'Missing params', atPhase: 'validate_plan' }],
        };
      }
      ...
      try {
        ...
        const statResult = (await statTool.invoke({ path: input.path })) as string;
        ...
        const escapedAnchor = searchAnchor.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        ...
        if (matchCount === 0) {
          throw new Error(`Could not find anchor "${searchAnchor}" in ${input.path}`);
        }
        ...
        logPhaseEnd('PLAN_VALIDATOR', `FAILED: step ${step.step_id}: ${errMsg}`, elapsed());
        return {
          phase: 'fatal',
          finalAnswer: `file_patch plan invalid: ${errMsg}`,
          errors: [{ code: 'invariant_violation', message: errMsg, atPhase: 'validate_plan' }],
        };
      }
    }
  }
  ...
  for (let i = 0; i < steps.length; i++) {
    if (steps[i]?.step_id !== i + 1) {
      ...
      return { phase: 'fatal', finalAnswer: 'Non-sequential step IDs.', errors: [] };
    }
  }
  ...
  for (const step of steps) {
    ...
    if (!tool) {
      ...
      return { phase: 'fatal', finalAnswer: `Unknown tool: ${step.tool}`, errors: [] };
    }
    ...
    if (schema?.safeParse) {
      ...
      if (!parsed.success) {
        ...
        return { phase: 'fatal', finalAnswer: `Invalid params for ${step.tool}`, errors: [] };
      }
    }
  }
