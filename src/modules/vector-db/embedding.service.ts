  ...
  if (!pipelinePromise) {
    pipelinePromise = import('@xenova/transformers').then(async (m) => {
      ...
      if (typeof pipeline !== 'function') {
        throw new Error('Failed to load embedding pipeline from @xenova/transformers');
      }
      // NOTE: This model is free and runs locally (no API key).
      return (pipeline as (task: string, model: string) => Promise<FeatureExtractionPipeline>)(
        'feature-extraction',
