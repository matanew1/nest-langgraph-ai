...
Array.prototype.sortBy = function <T>(this: T[], keySelector: (item: T) => any, order: 'asc' | 'desc' = 'asc'): T[] {
...
class Matcher<T, R> implements MatchBuilder<T, R> {
  ...
  default(result: R | (() => R)): R {
    ...
    return typeof handler === 'function' ? (handler as Function)() : handler;
  }
}
...
class TaskBuilder<T> implements SafeExecutionBuilder<T> {
  ...
  retry(count: number) { this._retries = count; return this; }
  onError(h: (err: Error) => void) { this._errCb = h; return this; }
  onFinish(h: () => void) { this._finishCb = h; return this; }
  fallback(v: T) { this._fallback = v; return this; }
  limit(ms: number) { this._limitMs = ms; return this; }
